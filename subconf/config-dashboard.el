...
(defun +dashboard-setup-modified-keymap ()
  ...
  (map! :map +dashboard-mode-map
        ...
        :desc "Show keybindings" :ng "h" (cmd! (which-key-show-keymap '+dashboard-mode-map))))

(add-transient-hook! #'+dashboard-mode (+dashboard-setup-modified-keymap))
(add-transient-hook! #'+dashboard-mode :append (+dashboard-setup-modified-keymap))
(add-hook! 'doom-init-ui-hook :append (+dashboard-setup-modified-keymap))

(map! :leader :desc "Dashboard" "d" #'+dashboard/open)

(defun +dashboard-benchmark-line ()
...
(setq +dashboard-functions
      (list #'+dashboard-widget-banner
            #'+dashboard-benchmark-line
            #'splash-phrase-dashboard-insert))

(defun +dashboard-tweak (&optional _)
  (with-current-buffer (get-buffer +dashboard-name)
...
(add-hook '+dashboard-mode-hook #'+dashboard-tweak)

(add-hook 'doom-after-init-hook #'+dashboard-tweak 1)

(setq +dashboard-name "► Doom"
      doom-fallback-buffer-name +dashboard-name)
