...
It only makes sense to do this on Linux systems where =dbus= can be loaded, so we
can check this at config generation time and pick whether it should be used.

#+name: conditional-auto-theme
#+begin_src emacs-lisp :noweb-ref none :noweb yes :noweb-prefix no
(if (and (memq system-type '(gnu gnu/linux gnu/kfreebsd))
         (require 'dbus nil t))
    (mapconcat
     #'pp-to-string
     '(<<auto-theme>>)
