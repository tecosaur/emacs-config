...
#+begin_src emacs-lisp :noweb-ref none :noweb yes :noweb-prefix no
(cond
...
Now, with the nice selection doom provides there's no reason for me to want the
defaults.
...
~+doom-init-theme-termaware~ while we're at it.
...
(org-export-update-features 'latex
  (accent-colour :snippet org-latex-accent-colours))
#+end_src
...
\cs_new:Npn \cover_print_date:n #1
  {\exp_args:Ne \cover_print_date_aux:n {#1}}

\cs_new:Npn \cover_print_date_aux:n #1 {%
  \bool_if:nTF
    {\int_compare_p:n {\str_count:n {#1} = 10} &&
     \str_if_eq_p:ee {\str_item:nn {#1} {5}} {-} &&
     \str_if_eq_p:ee {\str_item:nn {#1} {8}} {-}}
