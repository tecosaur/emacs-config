...
#+name: confpkg-create-depmap
#+begin_src emacs-lisp
(defun confpkg-create-depmap ()
  (let* ((doom-init ; ((:category modules...)...)
         ...
         (doom-module-trees ; ("/path/to/modules"...), highest priority first
          (or (bound-and-true-p doom-module-load-path)
              (error "Cannot locate Doom's module trees: `doom-module-load-path' is unset")))
         (doom-module-packages-file ; :category, module -> "/path/to/packages.el"
          (lambda (category module)
            (cl-some
             (lambda (tree)
               (let ((path (format "%s/%s/%s/packages.el"
                                   tree
                    ...
                    (mapcar
                     (lambda (module)
                       (let ((module-packages-path
                              (funcall doom-module-packages-file category module)))
                         (and module-packages-path
                              (cons module
                                    (with-temp-buffer
...
To actually apply this, we can run this after ~confpkg-tangle-finalise~ with
advice.

#+name: schedule-confpkg-create-depmap
#+begin_src emacs-lisp
(advice-add 'confpkg-tangle-finalise :after #'confpkg-create-depmap)
#+end_src

#+call: confpkg-create-depmap()
#+call: schedule-confpkg-create-depmap()
...
(:if (and (require 'battery) ; check if a mobile/laptop using the battery as a proxy
          battery-status-function
          (not (equal (alist-get ?L (funcall battery-status-function)) "N/A")))
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/font-face.png]]
...
from them based on the system theme, so let's record both preferences.
...
First though, let's see if the =DOOM_THEME= environment variable is set, and use
that (falling back on the dark theme).
...
While this is good, if the system switches between day and night modes, we want
to follow it. Thanks to the continued expansion of the =xdg-desktop-portal=
protocols, we can read this from D-Bus on most systems. We can even watch for
changes to boot.
...
(defun +doom-theme--dbus-update (namespace setting value)
  ...
  (when (and (string= namespace "org.freedesktop.appearance")
    ...
    (let* ((value (car value))
           (theme (+doom-theme--from-system value))
...
(when (and (display-graphic-p (selected-frame))
  ...
  (dbus-register-signal
   ...
   "SettingChanged"
   #'+doom-theme--dbus-update))
#+end_src

On MacOS we can also check the system theme, and there is a convenient
pre-existing =ns-system-*= variable and hook.

#+name: auto-theme-darwin
#+begin_src emacs-lisp :noweb-ref none
(defun +doom-theme--from-system (system-theme)
  "Pick a doom theme based on the SYSTEM-THEME symbol."
  (pcase system-theme
    ('dark doom-theme-dark)
    ('light doom-theme-light)
    (_ doom-theme-dark)))

(defun +doom-theme--ns-update (new-theme)
  "Handle a signal for system theme changes."
  (when (memq new-theme '(light dark))
    (let* ((theme (+doom-theme--from-system new-theme))
           (alt-value (if (eq new-theme 'light) 'dark 'light))
           (alt-theme (+doom-theme--from-system alt-value)))
      (when (memq alt-theme custom-enabled-themes)
        (mapc #'disable-theme custom-enabled-themes)
        (load-theme theme t)))))
...
(when (and (display-graphic-p (selected-frame))
           (not (getenv "DOOM_THEME")))
  (add-hook 'ns-system-appearance-change-functions #'+doom-theme--ns-update)
  (setq doom-theme (+doom-theme--from-system ns-system-appearance)))
#+end_src

We can now pick which flavour of adaptive theming we want at config generation
time.

#+name: conditional-auto-theme
#+begin_src emacs-lisp :noweb-ref none :noweb no-export :noweb-prefix no
(cond
...
I experienced some frustrating issues with theme race conditions, which seem to
be resolved by moving =doom-init-theme-h= around. Henrik attempted to help with
this in May 2021 but we didn't manage to pin down the issue. It may be worth
periodically checking back and seeing if this is still needed. We might as well
inject ~+doom-init-theme-termaware~ while we're at it.
...
***** Skipping the default Emacs themes

Lastly, with the nice selection doom provides there's no reason for me to want
the defaults.
...
FUNC shall be a function taking one argument."
  ...
  (setq emacs-lisp-mode-hook '(rainbow-delimiters-mode highlight-quoted-mode))
  ;; Pretty print
  (defun pp-sexp (sexp)
    (with-temp-buffer
      (cl-prettyprint sexp)
...
Now we can create the desired dashboard by setting ~+dashboard-functions~ to
just have:
...
~+dashboard-tweak~ as a slightly late init hook, after =doom-modeline= has been
loaded.
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/splash-screen.png]]
...
(if (file-exists-p "setup.sh")
  ...
  (pp-to-string
   `(unless noninteractive
      (add-hook! 'doom-init-ui-hook #'+literate-tangle--async))))
#+end_src
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/whichkey-evil.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/screenshot.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/info-colours.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/info-coloured.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/keycast.png]]
...
Some people like them, others don't. Personally, I'm a fan.

#+begin_src emacs-lisp :tangle packages.el
(package! rainbow-delimiters)
#+end_src

We'll just cap the depth a bit, and add it as a hook to the modes I appreciate
it in.
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/writeroom-and-org.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/nov.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/calc-with-calctex.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/elfeed.png]]
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/lexic.png]]
     ...
     (pp `(package! org
            :recipe (,@(if dev-key-p
                           (list :host nil :repo "tec@git.savannah.gnu.org:/srv/git/emacs/org-mode.git" :local-repo "lisp/org"
                                 :fork (list :host nil :repo "git@ssh.tecosaur.net:tec/org-mode.git" :branch "dev" :remote "tecosaur"))
...
[[https://code.tecosaur.net/tec/emacs-config/media/branch/master/misc/screenshots/org-capture.png]]
...
It's nice for documents to look a bit different from each other at a glance, and
one nice way to accomplish this is with accent colours. By using a good colour
space supported by =color.el=, we can bound the chroma and lightness, allowing
sampling of pleasant pastel-y shades. This can then be used to offer a few
shades of an accent colour, spicing up title pages and other suitable elements.

By default the accent hue is derived from the document title, giving each
document a consistent but distinct look. This can be controlled with
=#+accent_color: VALUE= where =VALUE= is one of:
+ =t= / =yes= :: use the default title-derived colour (the default)
+ =nil= / =no= :: produce grey (achromatic) shades
+ 0--360 :: use that as the hue angle directly (in degrees)
+ a colour name :: extract the hue from that colour
+ a hex colour :: extract the hue from that colour

This is registered as a dedicated keyword rather than an =#+options= toggle,
since the value space is richer than a simple boolean and this may be extended to
other export backends in the future.
...
The core shade generation works in OKLab space, taking a hue angle (in radians)
and a chroma factor (0.0 to 1.0) that scales the saturation.
...
Now we can parse the keyword value into a hue and chroma factor. The
title-derived case hashes the title to pseudo-randomly select both, preserving
the original behaviour. For a user-specified colour, the hue is extracted via
OKLab and the chroma factor is derived from the colour's actual chroma --- this
ensures the generated shades never exceed the saturation of the provided colour.
...
Finally, we tie everything together. The option is read from =info=, parsed into
a hue and chroma factor, and the five accent shades are generated.
...
When using the KOMA class, it's nice to have left-hanging sections with a dash
of colour, with a little colour in the TOC too.

#+name: org-latex-koma-sections-preamble
#+begin_src LaTeX
...
(defvar org-latex-koma-sections-preamble
  <<grab("org-latex-koma-sections-preamble")>>
...
+ Add a nice way of formatting a =YYYY-MM-DD= date
+ Redefine =\maketitle=
+ Draw an simplified version of my tree avatar, with [[https://xyz2tex.github.io/svg2tikz/][=svg2tikz=]]
