...
(let ((dev-key (and (file-exists-p "~/.ssh/id_ed25519.pub")
      ...
      (recipe-common '(:files ("*.el" "lisp/*.el")
                       :pre-build
  ...
  (prin1-to-string
   (cond ((and dev-key dev-pkg)
          `(package! org-mode
             :recipe (:local-repo ,(expand-file-name "straight/repos/org-mode" doom-local-dir) ,@recipe-common)
             :pin nil))
         (dev-key
          `(package! org-mode
             :recipe (:host nil :repo "git@code.orgmode.org:bzg/org-mode.git" ,@recipe-common)
             :pin nil))
         (t `(unpin! org-mode)))))
#+end_src
